...
import sys
import numpy as np
sys.path.append(os.path.join(os.path.dirname(__file__), '../../Semantic-SAM'))
...
def args_parser():
    ...
    parser.add_argument("--dataset_root", type=str, default="AVS_dataset", help="Absolute or relative path to AVS dataset root.")
    parser.add_argument("--sam_ckpt", type=str, default="", help="Path to Semantic-SAM checkpoint.")
    parser.add_argument("--level", type=int, default=2, help="Semantic-SAM level.")
    parser.add_argument("--num_process", type=int, default=1, help="Number of processes.")
    parser.add_argument(
    "--vis_mask",
    action="store_true",
...
def process_image_withPool(args, index_list, img_path, df_split, output ,vis_mask=False):
    ...
    if args.sam_type == "sam":
        ...
        cand1 = os.path.join(repo_root, 'Semantic-SAM', 'swinl_only_sam_many2many.pth')
        cand2 = os.path.join(repo_root, 'swinl_only_sam_many2many.pth')
        ckpt_path = args.sam_ckpt if args.sam_ckpt and os.path.isfile(args.sam_ckpt) else (cand1 if os.path.isfile(cand1) else cand2)
        if not os.path.isfile(ckpt_path):
            raise FileNotFoundError(f"Semantic-SAM checkpoint not found: {ckpt_path}")
        mask_generator = SemanticSamAutomaticMaskGenerator(build_semantic_sam(model_type='L', ckpt=ckpt_path), level=[args.level])
    ...
    for index in tqdm(index_list):
        ...
        if args.data_name == "s4":
            ...
            processed_dir = os.path.join(img_path, label_name, video_name, 'processed_frames')
            raw_dir = os.path.join(img_path, label_name, video_name, 'frames')
            img_base_path = processed_dir if os.path.isdir(processed_dir) else raw_dir
        ...
        for image_pth in targets:
            ...
            if args.sam_type in ("semantic_sam", "mobile_sam"):
...
if __name__ == "__main__":
    args = args_parser()
    if args.data_name == "s4":
        anno_csv = os.path.join(args.dataset_root, 'AVSBench_object/Single-source/s4_meta_data.csv')
        img_path = os.path.join(args.dataset_root, 'AVSBench_object/Single-source/s4_data/visual_frames')
    elif args.data_name == "ms3" :
        anno_csv = os.path.join(args.dataset_root, 'AVSBench_object/Multi-sources/ms3_meta_data.csv')
        img_path = os.path.join(args.dataset_root, 'AVSBench_object/Multi-sources/ms3_data/visual_frames')
    
    elif args.data_name == "avss" :
        anno_csv = os.path.join(args.dataset_root, 'AVSBench_semantic/metadata.csv')
        img_path = os.path.join(args.dataset_root, 'AVSBench_semantic')
    split = args.split  
