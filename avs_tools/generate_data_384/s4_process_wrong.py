...
def args_parser():
    ...
    parser.add_argument("--dataset_root", type=str, default="AVS_dataset")
...
if __name__ == '__main__':
    args = args_parser()
    ss_root = os.path.join(args.dataset_root, 'AVSBench_semantic')
    s4_root = os.path.join(args.dataset_root, 'AVSBench_object/Single-source/s4_data_384')
    ss_csv_path = os.path.join(args.dataset_root, 'AVSBench_semantic/metadata.csv')
    s4_csv_path = os.path.join(args.dataset_root, 'AVSBench_object/Single-source/s4_meta_data.csv')
    ...
    for index in range(len(df_v1s)):
        ...
        # pre mask
        pre_mask_dir_candidates = [
            os.path.join(ss_root, 'pre_SAM_mask/AVSBench_semantic/v1s', video_name, 'frames'),
            os.path.join(args.dataset_root, 'pre_SemanticSAM_mask/AVSBench_semantic/v1s', video_name, 'frames'),
            os.path.join(args.dataset_root, 'pre_SemanticSAM_mask/AVSBench_semantic/v1s', video_name, 'processed_frames'),
        ]
        pre_mask_dir = None
        for cand in pre_mask_dir_candidates:
            if os.path.isdir(cand):
                pre_mask_dir = cand
                break
        if pre_mask_dir is None:
            raise FileNotFoundError('pre mask directory not found for video {}'.format(video_name))
