...
def setup(args):
    ...
    cfg.merge_from_file(args.config_file) # 'configs/avs_ss/COMBO_SWINB_bs8_90k.yaml'
    cfg.merge_from_list(args.opts) # 临时修改某些配置项，而不需要更改配置文件
    cfg.freeze() # 将cfg变为只读
    default_setup(cfg, args) # 执行一系列标准的初始化操作
...
def main(args):
    ...
    try:
        ...
        if "Cannot match one checkpoint key to multiple keys in the model" in str(e):
            weights_path = cfg.MODEL.WEIGHTS
            if weights_path.endswith(".pkl"):
                from detectron2.utils.file_io import PathManager
                with PathManager.open(weights_path, "rb") as f:
                    data = pickle.load(f, encoding="latin1")
                if "model" in data:
                    state = data["model"]
                    converted = {}
                    for k, v in state.items():
                        nk = k if k.startswith("backbone.") else ("backbone." + k)
                        converted[nk] = v
                    trainer.model.load_state_dict(converted, strict=False)
        else:
            raise
