...
def setup(args):
    ...
    output_dir = os.path.join(cfg.OUTPUT_DIR + '/vis_beifen/')
...
def main(args):
    cfg = setup(args)
    if args.eval_only:
        ckpt_env = os.environ.get("CKPT_PATH", "")
        default_ckpt = os.path.join(cfg.OUTPUT_DIR, "model_best.pth")
        best_ckpt_path = args.ckpt if getattr(args, "ckpt", "") else (ckpt_env if ckpt_env else default_ckpt)
        print("Checkpoint path: {}".format(best_ckpt_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str, default="", help="Override checkpoint path for evaluation")
