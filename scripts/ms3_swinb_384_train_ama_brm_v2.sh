#!/bin/bash
# ============================================================
# MS3 + AMA + BRM v2 训练脚本 (针对过拟合优化)
# ============================================================
# 优化内容:
#   1. 减少迭代次数: 30000 → 12000 (峰值约在 5k-10k)
#   2. Batch size 4 (8 会 OOM), 学习率 0.00007
#   3. 增加评估频率: 1000 → 500 (更精确捕捉峰值)
#   4. 降低边界损失权重: 0.5 → 0.2 (防止 BRM 过拟合)
#
# 问题分析:
#   - MS3 只有 296 个训练视频，数据量太小
#   - 原 30000 iter 训练时间过长，导致严重过拟合
#   - 峰值在 iter 5k-10k，之后持续下降
# ============================================================

dataset_root="/media/a100/c5e1bf65-7974-432f-8aed-7a1345241efe/hhj/AVS_dataset/AVSBench_object/Multi-sources/"
export DETECTRON2_DATASETS=$dataset_root

# S4 预训练权重 (带 AMA)
s4_weights="/media/a100/c5e1bf65-7974-432f-8aed-7a1345241efe/hhj/VCT_AVS/output/s4_swinb_384_bs8_ama/model_best.pth"

echo "=== MS3 + AMA + BRM v2 Training (Anti-Overfitting) ==="
echo "Dataset: MS3 (296 train videos)"
echo "Batch Size: 4 (8 causes OOM even with AMP)"
echo "AMA: Enabled"
echo "BRM: Enabled (λ=0.2, reduced from 0.5)"
echo "Max Iter: 12000 (reduced from 30000)"
echo "Base LR: 0.00007 (lower to prevent overfitting)"
echo "Eval Period: 500 (increased frequency)"
echo "Initial Weights: $s4_weights"
echo ""

python train_net.py \
    --num-gpus 1 \
    --config-file configs/ms3_swinb_384/COMBO_SWINB.yaml \
    --dist-url tcp://0.0.0.0:47780 \
    MODEL.WEIGHTS ${s4_weights} \
    MODEL.MASK_FORMER.USE_AMA True \
    MODEL.MASK_FORMER.USE_BOUNDARY_REFINEMENT True \
    MODEL.MASK_FORMER.BOUNDARY_LOSS_WEIGHT 0.2 \
    SOLVER.IMS_PER_BATCH 4 \
    SOLVER.BASE_LR 0.00007 \
    SOLVER.MAX_ITER 12000 \
    SOLVER.CHECKPOINT_PERIOD 500 \
    TEST.EVAL_PERIOD 500 \
    OUTPUT_DIR output/ms3_swinb_384_ama_brm_v2

